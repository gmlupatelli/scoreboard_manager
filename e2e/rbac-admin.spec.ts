...
test.describe('System Admin - Dashboard Oversight', () => {
  test('should see owner filter dropdown in dashboard', async ({ adminAuth }) => {
    ...
    const ownerFilter = adminAuth.locator('text=Filter by Owner').or(
      adminAuth.locator('[placeholder*="owner"]')
    );
  ...
  test('should be able to filter scoreboards by owner', async ({ adminAuth }) => {
    ...
    const filterInput = adminAuth.locator('input[placeholder*="owner"]').or(
      adminAuth.locator('select[name="owner"]')
    );
    ...
    if (await filterInput.isVisible()) {
      ...
      const filterOptions = adminAuth.locator('[role="option"]').or(
        adminAuth.locator('option')
      );
...
test.describe('System Admin - Settings Management', () => {
  test('should access system settings page', async ({ adminAuth }) => {
    ...
    await expect(adminAuth.getByRole('heading', { name: /system settings/i }).or(
      adminAuth.locator('h1').filter({ hasText: /settings/i }).first()
    )).toBeVisible();
  });
  ...
  test('should see allow_public_registration setting', async ({ adminAuth }) => {
    ...
    const publicRegToggle = adminAuth.locator('text=Public Registration').or(
      adminAuth.locator('text=Allow Public Registration')
    );
  ...
  test('should see require_email_verification setting', async ({ adminAuth }) => {
    ...
    const emailVerificationToggle = adminAuth.locator('text=Require Email Verification').or(
      adminAuth.locator('text=Email Verification')
    );
...
test.describe('System Admin - Invitations Oversight', () => {
  ...
  test('should see inviter filter dropdown', async ({ adminAuth }) => {
    ...
    const inviterFilter = adminAuth.locator('text=Filter by Inviter').or(
      adminAuth.locator('[placeholder*="inviter"]')
    );
  ...
  test('should see invitations from multiple users', async ({ adminAuth }) => {
    ...
    const invitationsContainer = adminAuth.locator('table').or(
      adminAuth.locator('[data-testid="invitations-list"]')
    );
...
test.describe('System Admin - Scoreboard Access', () => {
  test('should be able to access another users scoreboard', async ({ adminAuth }) => {
    ...
    const firstScoreboard = adminAuth.locator('[data-testid="scoreboard-card"]').or(
      adminAuth.locator('.scoreboard-card')
    ).first();
  ...
  test('should be able to navigate to manage view of another users scoreboard', async ({ adminAuth }) => {
    await adminAuth.goto('/dashboard');
    await adminAuth.waitForTimeout(2000);
    
    // Find and click manage button on first scoreboard
    const manageButton = adminAuth.locator('text=Manage').or(
      adminAuth.locator('button:has-text("Edit")')
    ).first();
...
test.describe('System Admin - Navigation', () => {
  test('should see system admin navigation links', async ({ adminAuth }) => {
    ...
    const settingsButton = adminAuth.locator('button:has-text("Settings")').or(
      adminAuth.locator('a:has-text("Invitations")')
    );
  ...
  test('should be able to navigate to all admin pages', async ({ adminAuth }) => {
    ...
    const errorMessage = adminAuth.locator('text=Access Denied').or(
      adminAuth.locator('text=Forbidden')
    );
