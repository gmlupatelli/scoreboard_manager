...
test.describe('Invite-Only Mode - Toggle Feature', () => {
  ...
  test('admin should disable public registration', async ({ adminAuth }) => {
    ...
    const publicRegToggle = adminAuth.locator('input[type="checkbox"]').filter({
      has: adminAuth.locator('~ text=/Public Registration/i'),
    }).or(
      adminAuth.locator('label:has-text("Public Registration")').locator('input')
    ).or(
      adminAuth.locator('label:has-text("Allow Public Registration")').locator('input')
    ).first();
  ...
  test('registration page should require invitation code when disabled', async ({ page }) => {
    ...
    const invitationField = page.locator('input[name="invitationCode"]').or(
      page.locator('input[placeholder*="invitation code"]')
    );
    
    const invitationMessage = page.locator('text=/invitation.*required/i').or(
      page.locator('text=/invite.*only/i')
    );
    
    // Either invitation field or message should be visible in invite-only mode
    const hasInvitationUI = await invitationField.isVisible().catch(() => false) ||
...
test.describe('Invite-Only Mode - Enforcement', () => {
  test.beforeEach(async ({ adminAuth }) => {
    ...
    const publicRegToggle = adminAuth.locator('label:has-text("Public Registration")').locator('input').or(
      adminAuth.locator('input[type="checkbox"]').first()
    );
  ...
  test('unauthenticated user should see invitation requirement', async ({ page }) => {
    ...
    const inviteMessage = page.locator('text=/invite.*only|invitation.*required|registration.*restricted|invite.*registration/i');
    
    const inviteField = page.locator('input[name="invitationCode"]');
    const emailField = page.locator('input[name="email"]');
    
    const hasInviteUI = await inviteMessage.isVisible().catch(() => false) ||
  ...
  test('registration without invitation code should fail', async ({ page }) => {
    ...
    if (await emailField.isVisible() && await submitButton.isVisible()) {
      ...
      const registrationFailed = await errorMessage.isVisible().catch(() => false) || stillOnRegister;
...
test.describe('Invite-Only Mode - Settings Persistence', () => {
  test('setting changes should persist across page reloads', async ({ adminAuth }) => {
    ...
    if (await publicRegToggle.isVisible()) {
      ...
      const newPublicRegToggle = adminAuth.locator('label:has-text("Public Registration")').locator('input').or(
        adminAuth.locator('input[type="checkbox"]').first()
      );
