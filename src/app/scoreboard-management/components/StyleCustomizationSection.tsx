            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
              <div className="flex gap-3">
                ...
                <div className="text-sm text-gray-700">
                  <p className="font-medium mb-1">Tips for themes</p>
                  <ul className="list-disc list-inside space-y-1 text-gray-600">
                    <li>Preset themes are available on the Free plan</li>
                    <li>Custom theme edits require a Supporter plan</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {Object.keys(STYLE_PRESETS).map((preset) => {
                const isLocked = preset === 'custom' && !isSupporter;
                return (
                  <button
                    key={preset}
                    onClick={() => !isLocked && handlePresetChange(preset)}
                    disabled={isLocked}
                    ...
                    title={isLocked ? 'Supporter feature' : PRESET_LABELS[preset]}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {isLocked && (
                        <Icon name="LockClosedIcon" size={14} className="text-text-secondary" />
                      )}
                    </div>
                  ...
                  {CUSTOMIZABLE_PROPERTIES.map((prop) => (
                    <div key={prop.key} className="space-y-1">
                      ...
                      {prop.type === 'color' ? (
                        <ColorPicker
                          value={
                            getStyleValue(prop.key as keyof ScoreboardCustomStyles) || '#ffffff'
                          }
                          onChange={(color) =>
