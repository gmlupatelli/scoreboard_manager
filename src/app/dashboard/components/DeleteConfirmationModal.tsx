...
const DeleteConfirmationModal = ({ isOpen, scoreboardTitle, onConfirm, onCancel }: DeleteConfirmationModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onMouseDown={(e) => { if (e.target === e.currentTarget) onCancel(); }}>
      ...
      >
        <div role="dialog" aria-modal="true" aria-labelledby="delete-modal-title" className="relative bg-card border border-border rounded-lg p-4 sm:p-6 landscape-mobile:p-3 max-w-[calc(100vw-2rem)] sm:max-w-md w-full mx-4 elevation-3">
          <div className="flex items-start space-x-4">
            <div className="flex-shrink-0 w-12 h-12 rounded-full bg-destructive/10 flex items-center justify-center">
              <Icon name="ExclamationTriangleIcon" size={24} className="text-destructive" />
            </div>
            <div className="flex-1">
              <h3 id="delete-modal-title" className="text-lg font-semibold text-text-primary mb-2">Delete Scoreboard</h3>
