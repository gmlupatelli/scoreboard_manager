...
export const resourceService = {
  ...
  async createResource(resource: ResourceInsert) {
    try {
      const { data, error } = await supabase.from('table_name').insert(resource).select().single();
...
const { data, error } = await supabase.from('table_name').select('*').eq('column', value).single(); // Use .single() for one record
